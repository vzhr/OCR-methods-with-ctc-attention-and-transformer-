...
def train(train_dir=None, mode='train'):
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        print('len(labels):%d, batch_size:%d'%(len(train_feeder.labels), FLAGS.batch_size))
        num_batches_per_epoch = int(len(train_feeder.labels) / FLAGS.batch_size / len(gpus))
...
def infer(mode='infer'):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        with tf.Session(config=config) as sess, open(os.path.join(result_dir,  'result_digit_v1.txt'), 'w') as f:
            ...
            variable_to_restore = model.variable_averages.variables_to_restore()
